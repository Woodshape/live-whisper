...
Local transcription of live computer audio, existing audio/video files, or YouTube videos. For a YouTube link, the app first checks for a complete public caption track and uses it directly when available; otherwise it downloads audio to a temporary local file and transcribes it with [faster-whisper](https://github.com/SYSTRAN/faster-whisper) on your CPU. The Flutter desktop MVP has no browser or HTTP server: it starts a local Python worker over private stdin/stdout pipes. FFmpeg decodes local and downloaded media. Audio stays on your computer (YouTube media is fetched directly to the computer; the model downloads on first use).
...
The native window offers live capture, file selection without copying/uploading the original, YouTube URL import, output file selection, a saved Whisper model choice with an optional **Pre-load model** button, a saved **Deutsch (de)** / **English (en)** spoken-language selection (Deutsch by default, applied to live and file transcription; YouTube imports use the video's own spoken language instead, falling back to this selection when YouTube reports none), and a saved **Low latency (4s)** / **Higher accuracy (8s)** live preset. These preferences are restored between app sessions. Capture and Settings tabs separate the session controls from model/audio configuration. The app also shows Start/Stop, live status, speed/backlog indicators, and a scrollable preview of the last ten saved transcript lines (newest first). Low latency is the desktop default; the longer preset gives Whisper more context and may help a slower computer sustain real-time processing. The selection takes effect when you start a new live session; file transcription stays at 30-second chunks. Pre-load downloads and initializes the selected model without capturing audio; wait for **Model ready in memory** before starting a session to skip the model-loading wait. Only one model is held in memory; switching models replaces it, and closing the app releases it. If you skip Pre-load, Start loads the model automatically while buffering captured audio. Even with a warm model, the first live transcript line waits for the first 4- or 8-second chunk and inference. During file transcription, the audio-waiting indicator also shows an approximate ETA once recent processing speed is available (queued decoded audio ÷ speed). It may vary with the recording; live capture has no completion ETA. The preview resets when a new session starts and persists when the output file changes during a session. For an existing file choose **Choose audio / video**, optionally set an output path, then **Transcribe file**. File transcription reads the original recording without deleting it. For a YouTube video, paste a single video URL and choose **Check captions & transcribe**. Creator-written captions are always used when the video has them. Otherwise, with **Use YouTube's automatic captions** ticked, YouTube's automatic track is used and merged — its cues repeat the previous cue as they roll, so the app removes the repeats rather than duplicating the transcript. Untick it to skip automatic captions; those videos download audio temporarily and run the selected Whisper model, using the video's spoken language reported by YouTube rather than the saved live-capture language. Playlist links, live/upcoming videos, and videos longer than four hours are not supported. Availability depends on YouTube; use only content you are authorized to process. The Stop button ends capture or a YouTube import and processes audio already buffered. While the buffer is draining, **Cancel** completes the current 4- or 8-second live chunk, or 30-second file/YouTube-audio chunk, including its transcript write, then discards the remaining audio; transcript lines already written are kept. By default, each transcription starts a file at `~/YYYY-MM-DD_HH-MM` using the local start time (no seconds or extension); the active path appears in the status panel. Use the folder button to choose an output path, or clear the override to restore a fresh timestamped file on the next session. Files are append-only: two sessions started within the same minute share a path, and changing the path during a session sends future lines to the new file. Keep the app open until processing finishes.
...
Open **http://127.0.0.1:8765/** in your browser. Leave the output field blank for the timestamped default or explicitly enter a path such as `~/transcript.txt`, choose a model (`base` is a good starting point), then either select a device and start live transcription or select a recording and click **Upload & transcribe**. Stop with the Stop button or Ctrl+C in the terminal. To switch transcript files during a session, enter a new path and click **Set / change output**. Existing files are appended to, not overwritten. The output directory must exist.
...
**Note:** The desktop app writes an approximate timestamped line per ~4 or ~8 seconds of live audio, after model inference (the browser prototype continues to default to 8 seconds). File transcription uses ~30-second chunks. These are best-effort chunk timestamps, not word-level synchronized captions. Speech at chunk boundaries can be lost or repeated; there is no speaker identification. The desktop app fixes the selected language for each chunk instead of guessing again every 4 or 8 seconds; the browser prototype still auto-detects language. When public YouTube captions are used, their source language is retained and shown in status. Larger models improve accuracy but increase latency, especially on CPU. Uploaded files may be up to 4 GiB. YouTube links require internet access; local media can be transcribed offline after the selected model is cached.
